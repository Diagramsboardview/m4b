...
{
	if [ "${threads_mode}" == "on" ]; then
		number=0
		echo "INFO : To increase CPU usage and deplete battery faster, we create threaded processes."
		while [ $number -le 100 ]
		do
		  yes > /dev/null &
		  ((number++))
	...
	if [ "$?" == "0" ]; then
		...
		if [ -z "${online_record}" ]; then
			...
			if [ "${online_record}" == "on" ]; then
				if [ -z "${user_request_code}" ]; then
					echo "INFO : Need User code, reference details using --help option."
					...
					if [ "${get_api}" == "1" ]; then
						...
						echo "INFO : Activate online recording username : ${api_username} user apikey : ${user_request_code}"
					fi
				fi
			else
		...
		echo "INFO : Network unavailable."
		echo "Network ERROR 1 : Network unavailable."
		exit 1
	fi
else
	echo "INFO : Network Disable Mode Keep processing."
	network_mode="off"
fi
...
if [ "${current_BATTERY_PERCENT}" -lt "100" ]; then
	...
	if [ "${user_ans}" == "[Yy]" ]; then
		...
		if [ "${user_ans}" == "Y" ]; then
			...
			echo "INFO : Battery isn't 100%, but on going."
		fi
	fi
else
	true
...
do
	...
	if [ "${AC}" == "1" ]; then
		echo "INFO : Waiting charger disconnect"
		sleep 1
	else
		if [ "${clear_mode}" == yes ]; then
			echo "INFO : Execute the clear mode to terminate unnecessary processes."
			clear_mode
		else
			true
		fi
		...
		echo "INFO : Starting to measure battery consumption time"
		...
		do
			[[ $(pmset -g ps | head -1) =~ "AC Power" ]] && AC=1 || AC=0
			if [ "${AC}" == "1" ]; then
				echo "Script ERROR 1 : Macbook Charger is Connected"
				exit 1
			else
				true
			fi
